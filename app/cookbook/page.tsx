...
export default function Cookbook(){
  ...
  useEffect(()=>{if(new URLSearchParams(window.location.search).get("add")==="recipe")setShowOwnRecipe(true);fetch("/api/meals").then(r=>r.json()).then(data=>{setSignedIn(Boolean(data.signedIn));setSaved(data.selectedRecipeIds??[]);setCustomCount(data.customRecipes?.length??0)}).catch(()=>setNotice("We couldn't load your saved meals."))},[]);
  ...
  return <main className="cookbook">
    <header className="book-top"><a href="/account">← My account</a><div><span>D</span><strong>DRIFTLINE COOKBOOK</strong><small>Choose dishes for your household</small></div><a className="book-account" href="/account">My meal plan</a></header>
    ...
    <div className="book-layout">
      <section className="recipe-gallery">{filtered.map(r=><article key={r.id} className={selected.id===r.id?"active":""} onClick={()=>{setSelected(r);setPortions(12)}}><button className={saved.includes(r.id)?"save on":"save"} aria-label={saved.includes(r.id)?"Remove from my meals":"Choose this dish"} onClick={e=>{e.stopPropagation();toggleMeal(r.id)}}>{saved.includes(r.id)?"✓":"+"}</button><img src={r.image} alt={r.title}/><div><small>{r.category} · #{String(r.id).padStart(3,"0")}</small><h2>{r.title}</h2><p>{r.active} min active · {r.total} min total</p><span>{r.tags.slice(0,2).map(t=><i key={t}>{t}</i>)}</span></div></article>)}</section>
