...
export async function createEvent(createdBy:string,event:Omit<ScheduleEvent,"id"|"createdAt"|"updatedAt">){await ensure();const now=new Date().toISOString();const result=await database().prepare("INSERT INTO schedule_events (service_date,start_time,end_time,household,customer_email,dishes,chef,chef_email,package_name,location,status,chef_pay_cents,notes,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(event.serviceDate,event.startTime,event.endTime,event.household,event.customerEmail,JSON.stringify(event.dishes),event.chef,event.chefEmail,event.packageName,event.location,event.status,event.chefPayCents,event.notes,createdBy,now,now).run();return{id:Number(result.meta.last_row_id),...event,createdAt:now,updatedAt:now}}
export async function updateEvent(id:number,event:Omit<ScheduleEvent,"id"|"createdAt"|"updatedAt">){await ensure();const now=new Date().toISOString();await database().prepare("UPDATE schedule_events SET service_date=?,start_time=?,end_time=?,household=?,customer_email=?,dishes=?,chef=?,chef_email=?,package_name=?,location=?,status=?,chef_pay_cents=?,notes=?,updated_at=? WHERE id=?").bind(event.serviceDate,event.startTime,event.endTime,event.household,event.customerEmail,JSON.stringify(event.dishes),event.chef,event.chefEmail,event.packageName,event.location,event.status,event.chefPayCents,event.notes,now,id).run();return{id,...event,createdAt:"",updatedAt:now}}
